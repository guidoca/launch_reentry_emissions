"""Tests for the plume_postcombustion emission index script."""
...
def test_black_carbon_postcombustion_stratosphere() -> None:
    ...
    h = 20.0e3
    result = PlumeEmissionIndexes.final_black_carbon_stratosphere(h)
    assert EI_BC_RP1_P * EI_BC_RED < result < EI_BC_RP1_P
...
def test_black_carbon_postcombustion_troposphere_array() -> None:
    ...
    result_array = PlumeEmissionIndexes.final_black_carbon(h_array)
    bc_min = EI_BC_RED * EI_BC_RP1_P
    np.testing.assert_allclose(result_array[0], bc_min)
    np.testing.assert_allclose(result_array[1], bc_min)
    assert bc_min < result_array[2] < EI_BC_RP1_P
    assert bc_min < result_array[3] < EI_BC_RP1_P
    np.testing.assert_allclose(result_array[-1], EI_BC_RP1_P)
...
def test_carbon_monoxide_postcombustion() -> None:
    ...
    h = 90.0e3
    ei_co_p = 0.1
    result = PlumeEmissionIndexes.final_carbon_monoxide(h, ei_co_p, 0.3)
...
def test_carbon_monoxide_postcombustion_sea_level() -> None:
    ...
    ei_co_f = PlumeEmissionIndexes.final_carbon_monoxide(h, ei_co_p, ei_co2_p)
...
def test_carbon_monoxide_postcombustion_srm() -> None:
    ...
    ei_co_f = PlumeEmissionIndexes.final_carbon_monoxide(h, ei_co_p, ei_co_p)
...
def test_nitrogen_oxides_postcombustion_sea_level() -> None:
    ...
    ei_nox_f = PlumeEmissionIndexes.final_nitrogen_oxides(h, ei_nox_p, ei_nox_s_sl)
...
def test_nitrogen_oxides_postcombustion_array() -> None:
    ...
    ei_nox_s_sl = rng.uniform(0.1, 0.4, size=size)
    ei_n_expected = ei_nox_p + ei_nox_s_sl
    ei_n_f = PlumeEmissionIndexes.final_nitrogen_oxides(h, ei_nox_p, ei_nox_s_sl)
...
def test_nitrogen_oxides_postcombustion_decreasing() -> None:
    ...
    ei_n_expected = ei_nox_p + EI_NOX_S_SL
    ei_n_f = PlumeEmissionIndexes.final_nitrogen_oxides(h, ei_nox_p)
...
def test_carbon_dioxide_postcombustion_none() -> None:
    ...
    h = rng.uniform(0.0, 90e3, size=size)
    ei_co2_p = 0.0
    ei_co2_f = PlumeEmissionIndexes.final_carbon_dioxide(
...
def test_water_vapor_postcombustion_none() -> None:
    ...
    h = rng.uniform(0.0, 90e3, size=size)
    ei_h2o_p = 0.0
    ei_h2o_f = PlumeEmissionIndexes.final_water_vapor(
...
def test_plume_postcombustion_model_sl() -> None:
    ...
    primary_emission_indexes = PrimaryEmissionIndexes(
        bc=EI_BC_RP1_P, co=0.1, co2=0.1, nox=0.1, h2o=0.1
    )
    plume_postcombustion = PlumeEmissionIndexes(
        primary_emission_indexes=primary_emission_indexes
    )

    final_emission_indexes = plume_postcombustion.evaluate(h=h)

    np.testing.assert_allclose(final_emission_indexes.bc, EI_BC_RP1_P * EI_BC_RED)
    ...
    np.testing.assert_allclose(
        ...
        / MolarMassSpecies.bc
        * (EI_BC_RP1_P - EI_BC_RP1_P * EI_BC_RED),
    )
...
def test_plume_postcombustion_model_from_csv(
    ...
    """Test PlumePostcombustionModel function with a single float value."""
    h = 0.0e3
    plume_postcombustion = PlumeEmissionIndexes.from_csv(
...
def test_generic_tuples() -> None:
    """Test the generic emission indexes."""
    kerolox = GenericKeroloxPrimaryEmissionIndexes()
    hydrolox = GenericHydroloxPrimaryEmissionIndexes()
    methalox = GenericMethaloxPrimaryEmissionIndexes()
    # No nitrogen oxides primary emissions
    assert kerolox.nox == 0.0
    assert hydrolox.nox == 0.0
    assert methalox.nox == 0.0
    # Hydrolox has no carbon emissions
    assert hydrolox.bc == 0.0
    assert hydrolox.co == 0.0
    assert hydrolox.co2 == 0.0
    assert hydrolox.ch4 == 0.0
    # Black carbon emissions in methalox lower than kerolox
    assert methalox.bc < kerolox.bc
    # Methalox has lower carbon emissions than kerolox
    assert methalox.co2 < kerolox.co2
    assert methalox.co < kerolox.co
    # Hydrolox has higher water emissions
    assert hydrolox.h2o > kerolox.h2o
    assert hydrolox.h2o > methalox.h2o

    # Check that totals are approximately 1 within 5%
    # No need for this to be exact.. trace emissions are not well known
    assert np.sum(hydrolox) == pytest.approx(1.0, rel=0.05)
    assert np.sum(kerolox) == pytest.approx(1.0, rel=0.05)
    assert np.sum(methalox) == pytest.approx(1.0, rel=0.05)
