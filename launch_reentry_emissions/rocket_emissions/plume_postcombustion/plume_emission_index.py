...
class GenericKeroloxPrimaryEmissionIndexes(typing.NamedTuple):
    """Primary emission indexes for a generic kerolox rocket."""

    nox: float = 0.0
    co2: float = 0.4512
    co: float = 0.2547094
    ch4: float = 0.0
    bc: float = EI_BC_RP1_P
    h2o: float = 0.2878
    h: float = 0.0
    h2: float = 0.005445698
    oh: float = 0.017816582


class GenericMethaloxPrimaryEmissionIndexes(typing.NamedTuple):
    """Primary emission indexes for a generic methalox rocket."""

    nox: float = 0.0
    co2: float = 0.312398952
    co: float = 0.168849267
    ch4: float = 0.054705846
    bc: float = EI_BC_CH4_P
    h2o: float = 0.425603464
    h: float = 0.0
    h2: float = 0.005445698
    oh: float = 0.017816582


class GenericHydroloxPrimaryEmissionIndexes(typing.NamedTuple):
    """Primary emission indexes for a generic hydrolox rocket."""
    ...
    ch4: float = 0.0
    bc: float = 0.0
    h2o: float = 0.928410156
    h: float = 0.0
    h2: float = 0.047705798
    oh: float = 0.01916065
...
class PlumeEmissionIndexes:
    ...
    def __init__(
        ...
        Args:
            ...
            nox_s_sl: Secondary NOx emission index at sea level. Defaults to EI_NOX_S_SL.

        """
        self.h = h
        self.primary_emission_indexes = primary_emission_indexes
        self.nox_s_sl = nox_s_sl
    ...
    def evaluate(
        ...
        return FinalEmissionIndexes(
            nox=PlumeEmissionIndexes.final_nitrogen_oxides(
                h, primary_emission_indexes.nox, ei_nox_s_sl=self.nox_s_sl
            ),
            co2=PlumeEmissionIndexes.final_carbon_dioxide(
                h,
            ...
            co=PlumeEmissionIndexes.final_carbon_monoxide(
                h, primary_emission_indexes.co, primary_emission_indexes.co2
            ),
            ch4=PlumeEmissionIndexes.final_carbon_monoxide(
                h, primary_emission_indexes.ch4, primary_emission_indexes.co2
            ),
            bc=PlumeEmissionIndexes.final_black_carbon(h, primary_emission_indexes.bc),
            h2o=PlumeEmissionIndexes.final_water_vapor(
                h,
            ...
            h=PlumeEmissionIndexes.final_carbon_monoxide(
                h,
                primary_emission_indexes.h,
                primary_emission_indexes.h2o,
            ),  # Assumed similar to carbon monoxide
            h2=PlumeEmissionIndexes.final_carbon_monoxide(
                h,
                primary_emission_indexes.h2,
                primary_emission_indexes.h2o,
            ),  # Assumed similar to carbon monoxide,
            oh=PlumeEmissionIndexes.final_carbon_monoxide(
                h,
                primary_emission_indexes.oh,
                primary_emission_indexes.h2o,
            ),  # Assumed similar to carbon monoxide,,
    ...
    def final_carbon_dioxide(
        ...
        ei_from_ch4_f = ei_ch4_p - PlumeEmissionIndexes.final_carbon_monoxide(
            h, ei_ch4_p, ei_co2_p
        )
        # Calculate the co2 emissions from black carbon post-combustion
        ei_from_bc_f = ei_bc_p - PlumeEmissionIndexes.final_black_carbon(h, ei_bc_p)

        # Calculate the co2 emissions from carbon monoxide post-combustion
        ei_from_co_f = ei_co_p - PlumeEmissionIndexes.final_carbon_monoxide(
            h, ei_co_p, ei_co2_p
        )
        return (
            ei_co2_p
    ...
    def final_water_vapor(
        ...
        ei_from_h_f = ei_h_p - PlumeEmissionIndexes.final_carbon_monoxide(
            h,
            ei_h_p,
            ei_h2o_p,
        )
        ...
        ei_from_h2_f = ei_h2_p - PlumeEmissionIndexes.final_carbon_monoxide(
            h,
            ei_h2_p,
            ei_h2o_p,
        )
        ...
        # We assume similar relation as to carbon monoxide, as no model is available.
        # This is a simplification, as the reaction of hydrogen with oxygen is more complex.
        ei_from_oh_f = ei_oh_p - PlumeEmissionIndexes.final_carbon_monoxide(
            h,
            ei_oh_p,
            ei_h2o_p,
        )
    ...
    def final_black_carbon_stratosphere(
        h: npt.NDArray[np.floating] | float, ei_bc_p: float = EI_BC_RP1_P
    ) -> npt.NDArray[np.floating] | float:
    ...
    def final_black_carbon(
        ...
        bc_high = np.minimum(
            ...
            PlumeEmissionIndexes.final_black_carbon_stratosphere(h, ei_bc_p),
        )
    ...
    def final_carbon_monoxide(
        ...
        ei_co_f_lower = PlumeEmissionIndexes.final_carbon_monoxide_lower(
